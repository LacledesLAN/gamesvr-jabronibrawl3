...
#####################################################################################################
### CONFIG VARS #####################################################################################
declare LLTEST_CMD="/app/srcds_run.sh -game jbep3 +map jb_arcane +maxplayers 8 -insecure -norestart +sv_lan 1";
declare LLTEST_NAME="gamesvr-jb3-$(date '+%H%M%S')";
#####################################################################################################
#####################################################################################################
...
#####################################################################################################
### TESTS ###########################################################################################
should_have 'Using default binary: ./bin/linux64/jbep3' 'Server started';

should_lack 'Server restart in 10 seconds' 'Server is not boot-looping';
should_lack '/app/.steam/sdk64/steamclient.so: cannot open shared object file: No such file or directory' 'steamclient.so not found!';
